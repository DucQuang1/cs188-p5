        ...
        for c in Cgrid:
            ...
            for iteration in range(self.max_iterations):
                ...
                for i in range(len(trainingData)):
                    ...
                    label = score.argMax()
                    if label != realLabel:
                        squaredDatum =datum.copy()
                        for f in squaredDatum:
                            squaredDatum[f]*=squaredDatum[f]
                        tau = min(c, ((cWeights[c][label]-cWeights[c][realLabel]) * datum + 1.0) / (2 * squaredDatum.totalCount()))
                        tempDatum=datum.copy()
                        for feat in tempDatum:
                            tempDatum[feat] *= tau
                        cWeights[c][label] -= tempDatum
                        cWeights[c][realLabel] += tempDatum
